// ==========================================
//  ASSEMBLY APP BACKEND (V30 - Full System with History)
// ==========================================
...
// (⚠️ เช็คชื่อไฟล์ .json ของคุณให้ถูกต้อง!)
...
// (สำคัญ) เสิร์ฟไฟล์หน้าบ้าน (HTML/CSS/JS)
...
// 8. ดึงยอด Operator (รวม Rack Logic + กรอง Model)
...
// 11. ดึงรายการ Rework ค้าง (API แยก - เผื่อหน้า rework_check.html ใช้)
...
// --- (V30 ใหม่!) /get-rework-history (ดึงประวัติงานซ่อมทั้งหมด) ---
app.get('/get-rework-history', async (req, res) => {
  try {
    ...
    const query = {
      ...
      $or: [
        { status: 'REWORK' }, 
        { rework_checked_at: { $exists: true } } 
      ]
    };

    const history = await db.collection('qc_log')
      .find(query)
      .sort({ timestamp: -1 })
      .toArray();
    ...
    console.error(err);
    res.status(500).send({ error: 'History Error' });
  }
});
...
async function startServer() {
  ...
  // (เปิด Port 0.0.0.0 เพื่อให้ Render และวง LAN มองเห็น)
  app.listen(PORT, '0.0.0.0', () => console.log(`✅ Server (V30 Full) running on port ${PORT}`));
}
startServer();
